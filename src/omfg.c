...
float	get_v_inter(t_mlx *mlx, float angl, int *lol)	// get the vertical intersection
{
	float	v_x;
	float	v_y;
	float	x_step;
	...
	if (*lol)
	{
		printf("current angle is %lf\n", angl);
		printf("VALUE OF y_STEP IMMEDIATELY WFTER CALCULATING TAN: %lf\n", y_step);
	}
	...
	if (*lol)
	{
		printf("VALUE OF V_Y FOR MIDRAY %lf and VALUE OF V_X FOR MIDRAY %lf \n",  v_y, v_x);	
	}
...
void	cast_rays(t_mlx *mlx)	// cast the rays
{
	...
	while (ray < S_W) // loop for the rays
	{
		...
		v_inter = get_v_inter(mlx, nor_angle(mlx->ray->ray_ngl), &lol); // get the vertical intersection
...
void init_the_player(t_mlx mlx)	// init the player structure
{
	...
	mlx.ply->angle = 3 * M_PI /2; // player angle
